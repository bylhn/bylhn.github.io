...
const BlogList = ({ onPost }) => {
  ...
  useEffect(() => {
    fetch('/data/posts.json').then(r => r.json()).then(data => { setPosts(data); setLoading(false) }).catch(() => setLoading(false))
  }, [])
...
const BlogPost = ({ slug, onBack }) => {
  ...
  const [loading, setLoading] = useState(true)
  useEffect(() => {
    fetch(`/data/posts/${encodeURIComponent(slug)}.json`).then(r => r.json()).then(data => { setPost(data); setLoading(false) }).catch(() => setLoading(false))
  }, [slug])
